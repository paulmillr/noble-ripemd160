...
describe("ripemd160", () => {
  test("Raw string ''", () => {
    ...
    const actualHash = ripemd160(rawString);
    expect(actualHash).toBe(expectedHash);
  });
  test("Uint8Array string ''", () => {
    const message = new Uint8Array();
    const expectedHash = new Uint8Array([156, 17, 133, 165, 197, 233, 252, 84, 97, 40, 8, 151, 126, 232, 245, 72, 178, 37, 141, 49]);
  ...
  test("Uint8Array string 'a'", () => {
    ...
    const expectedHash = new Uint8Array([11, 220, 157, 45, 37, 107, 62, 233, 218, 174, 52, 123, 230, 244, 220, 131, 90, 70, 127, 254]);
    const actualHash = ripemd160(message);
    expect(actualHash).toEqual(expectedHash);
  });
  test("Raw string 'abc'", () => {
  ...
  test("Uint8Array string 'abc'", () => {
    ...
    const expectedHash = new Uint8Array([142, 178, 8, 247, 224, 93, 152, 122, 155, 4, 74, 142, 152, 198, 176, 135, 241, 90, 11, 252]);
    const actualHash = ripemd160(message);
    expect(actualHash).toEqual(expectedHash);
  });
  ...
  test("Uint8Array string 'message digest'", () => {
    const message = new Uint8Array([109, 101, 115, 115, 97, 103, 101, 32, 100, 105, 103, 101, 115, 116]);
    const expectedHash = new Uint8Array([93, 6, 137, 239, 73, 210, 250, 229, 114, 184, 129, 177, 35, 168, 95, 250, 33, 89, 95, 54]);
    const actualHash = ripemd160(message);
    expect(actualHash).toEqual(expectedHash);
  });
